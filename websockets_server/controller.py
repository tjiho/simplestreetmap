...
class Controller():
    def __init__(self, database_connection) -> None:
        ...
        self.SessionMaker = sessionmaker(
            self.database_connection, expire_on_commit=False)
    ...
    def load_plan_by_read_token(self, read_token):
        with self.SessionMaker() as session:
            plan = session.query(Plan).filter(
                Plan.read_token == read_token).one()
            if plan is None:
                raise Exception("Plan not found")
            self.plan_id = plan.id
            self.plan_token = plan.token
    ...
    def remove_place(self, place_id):
        with self.SessionMaker() as session:
            try:
                place_object = session.query(Place).filter(
                    Place.uuid == place_id).one()
    ...
    def update_annotation(self, id, annotation):
        if self.can_edit == False:
            return None
        if annotation['object_type'] == 'place':
            return self.update_place(id, annotation)
