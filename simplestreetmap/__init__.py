...
def make_app():
    ...
    with Configurator() as config:
        ...
        config.add_view(reverse_view, route_name='reverse',
                        request_method='GET', renderer='json')
        config.add_view(itinerary_view, route_name='itinerary',
                        request_method='GET', renderer='json')

        config.add_request_method(
            lambda request: search_adapter, 'search_adapter', reify=True)
        config.add_request_method(
            lambda request: navitia_adapter, 'navitia_adapter', reify=True)
        config.add_request_method(
            lambda request: brouter_adapter, 'brouter_adapter', reify=True)
        config.add_request_method(
            lambda request: ssm_config, 'ssm_config', reify=True)
